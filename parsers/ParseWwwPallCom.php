...
class SiteParsers_ParseWwwPallCom extends Extractor {
	...
	function parse_product_content($page_html = '', $url)
	{
		...
		// scrape title
		$matches = array();
		preg_match("/<h1 id=\"product_name\"(.*?)>(.*?)<\/h1>/si", $page_html, $matches);
		...
		// scrape sku
		$matches = array();
		preg_match("/<div(.*?)id=\"ds_section_9\">(.*?)><\/div>/si", $page_html, $matches);

		preg_match_all("/<tr(.*?)class=\"content_row(.*?)\"(.*?)>(.*?)<\/tr>/si", $matches[2], $matches);

		foreach($matches[4] as $match){
			preg_match_all("/<td(.*?)>(.*?)<\/td>/si", $match, $matches);
			if (isset($matches[2]) && is_array($matches[2])){
				...
				if(isset($matches[2][2])) {
					$weight = $this->_calculate_weight($matches[2][2]);
				}else {
					$weight = "";
				}
			}
...
//		die(print_r($product));
		...
		return array("product" => $product, "product_additional_fields" => $prod_additional_fields);
	}
